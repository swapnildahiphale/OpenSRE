...
Safety note:
- Prefer `bkt auth login ... --web` for interactive credential setup.
- Never paste real tokens into shared chat/log output.
- Avoid `--allow-insecure-store` unless explicitly required and risk-accepted.
...
bkt extension install <repo>              # Install from a git repository
bkt extension install my-org/bkt-hello
...
OpenSRE primary auth (config-free; set in repo `.env`, passed to sre-agent):

- `BKT_TOKEN` — API token / PAT (bypasses keyring)
- `BKT_HOST` — `https://bitbucket.org` (Cloud) or DC base URL
- `BKT_USERNAME` — Cloud: Atlassian email; DC: PAT owner username
- `BKT_WORKSPACE` — Default Cloud workspace
- `BKT_PROJECT` — Default DC project key
- `BKT_REPO` — Default repository slug

Other:

- `BKT_CONFIG_DIR` — Config directory override (for `bkt auth login` / contexts)
- `BKT_ALLOW_INSECURE_STORE` — Allow file-based credential storage (set to `1`)
- `BKT_AUTH_METHOD` — `basic` or `bearer` (DC defaults to bearer without username)
